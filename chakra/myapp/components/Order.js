...
export default function Order (props) {
     ...
     switch (action.type) {
      ...
      case 'contactNumber':
        return {
         ...
         , errors : {...state.errors , contactNumber : {error : action.payload.length === 11 ? undefined : '🤯' , message : `${11 - action.payload.length} more to go` }}
        }  
      case 'email':
        return {
          ...state 
      ...
      case 'addressGoogle':
        return {
           ...
           , errors : {...state.errors , addressGoogle : {error : undefined  , message : 'Contact Number seems like invalid' }}
        }      
     }
    }
    ...
    async function handleUploadOrder () {
      ...
      let totalBill =  0
      for (let product of context.products) totalBill += isNaN (parseInt(product.totalBill)) ? 0 : parseInt(product.totalBill)
      
      if (totalBill === 0) {
        Toaster (toast , 'Product are out of stock please check out your cart' , 'error')
        setBtnLoading (false)
        return 
      }
      ...
      const requiedFields = ['userName' , 'address' , 'city' , 'country' , 'email' , 'contactNumber' ] 
      ...
      const res  = await superPower (`${process.env.NEXT_PUBLIC_SERVER_URL}/order/` , 'POST' , {
    ...
    useEffect (()=> {
        dispatch ({type : 'country' , payload : 'Pakistan'})
         ...
         async function makeReq () {
          ...
          try {
            var {data} = await axios.get (`https://ipinfo.io?token=${token}`)
    ...
    return (
      <>
        {loading ? <> Loading </> : <>
        <OrderDetails products = {context.products} cart = {context} mode = 'Checkout'/>
        <CheckOutForm state={state} dispatch = {dispatch}/>
        <Stack direction="row" alignItems="center" justifyContent={'center'} pb = {5}>
            <MdLocalShipping />
          <Text>2-3 business days delivery</Text>
        </Stack>
        <Center>
        <Flex>
          <Button colorScheme={'facebook'} mb = {'5'} onClick = {()=> handleUploadOrder()}
...
export function OrderDetails (props) {
   ...
   <Tbody>
      {props.products && props.products.map ( (product , idx) => <OrderMeta key = {idx} product = {product} mode = {props.mode} context = {props.cart} /> )}
      <Tr bg = {'linkedin.100'}>
      <Th>{` `}</Th>
      <Th>{` `}</Th>
      <Th>{` `}</Th>
...
function OrderMeta ({product , context , mode}) {
    ...
    return ( 
        ...
        <Tr  bg = {'gray.100'} _hover = {{bg : 'white'}} cursor = {'pointer'} key = {product.id}>
