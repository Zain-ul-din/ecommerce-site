...
export default function ProductsDashBoard(props) {
  ...
  function reRenderHook(id) {
    setProducts(products.filter((product_itr) => product_itr.id !== id));
  }
...
function ProductColumn({ product, reRenderHook }) {
  ...
  return (
    <Tr>
      <Td>
        <Link
          href={`${process.env.NEXT_PUBLIC_SERVER_URL}/static/${product.image}`}
          target={"_blank"}
        >
          <Image
            src={`${process.env.NEXT_PUBLIC_SERVER_URL}/static/${product.image}`}
            alt={"loading"}
            width={30}
            height={30}
            cursor={"pointer"}
              ...
              <Stack>
                <Button
                  ...
                  onClick={async (e) => {
                    ...
                    setLoading(true);
                    await superPower(
                      `${process.env.NEXT_PUBLIC_SERVER_URL}/product/${product.id}`,
                      "DELETE"
                    );
                    setLoading(false);
