...
export default function middleware (req) {
    ...
    if (adminPages.find (p => p === req.nextUrl.pathname)) {
      const authUser = getUserAuth (req.cookies[cookieName])
        if (authUser !== 'admin') return NextResponse.redirect( new URL(`/login`, req.url) )
    }
    
    // matchs dynamic pages
    if (adminDynamicPages.find(p => req.nextUrl.pathname.includes (p) && req.nextUrl.pathname.length > p.length )) {
      const authUser = getUserAuth (req.cookies[cookieName])
      if (authUser !== 'admin') return NextResponse.redirect( new URL(`/login`, req.url) )
    }
