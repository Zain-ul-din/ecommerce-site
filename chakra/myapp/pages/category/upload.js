...
export async function getServerSideProps(context) {
    const { data } = await superPower (`${process.env.NEXT_PUBLIC_SERVER_URL}/category` , 'GET')
...
function SubReducer (state , action) {
    switch (action.type) {
        case 'category':
            return {...state , 
                data : {...state.data , superCategory_id : action.payload.id , superCategoryName : action.payload.name } ,
                errors : {...state.errors , category : { error : undefined , message : '' }}
            }
        case 'name':
            return {...state , data : {...state.data , name : action.payload}
            , errors : {...state.errors , name : {error : action.payload.length > 1 ? undefined : '🎇', message : 'name is too short'}}
        }    
        case  'image':
            return {
...
export default function Upload (props) {
    ...
    async function handleUpload () {
        ...
        await uploadFile(state.data.image, async (imageUrl) => {
            ...
            const res = await superPower (`${process.env.NEXT_PUBLIC_SERVER_URL}/category` ,'POST' ,{
                category : {...state.data , image : imageUrl.data}
            })
            .catch(async (err) => {
                await superPower (`${process.env.NEXT_PUBLIC_SERVER_URL}/static/${imageUrl.data}` , 'DELETE')
    ...
    async function uploadSubCategory () {
        ...
        await uploadFile(subState.data.image, async (imageUrl) => {
            ...
            const res = await superPower (`${process.env.NEXT_PUBLIC_SERVER_URL}/subcategory` ,'POST' ,{
    ...
    <Menu >
    <MenuButton as={Button} variant = {'solid'} size= {'lg'} rightIcon={ subState.data.superCategory_id !== undefined  ? <CheckIcon/> : <ChevronDownIcon />}
     outlineColor = {subState.data.superCategory_id !== undefined ? 'green.400' : 'red.200'}
     color = {subState.data.superCategory_id !== undefined ? 'green' : 'black'}
    >
     Select Category
    </MenuButton>
    <MenuList>
     {categories && categories.map ((category , key)=>{
        return (<MenuItem 
            key = {key} _hover = {{opacity : 80}}
            onClick = {()=>subDispatch({type : 'category' , payload : category})}
            icon = {<CheckIcon opacity={subState.data?.superCategory_id === category.id ? 1 : 0}/>}
