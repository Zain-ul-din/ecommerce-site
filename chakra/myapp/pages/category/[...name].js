...
export async function getServerSideProps(context) {
  ...
  // Category Page
  if (params.length === 1) {
    const category = await superPower(
      `${process.env.NEXT_PUBLIC_SERVER_URL}/category/${params[0]}`,
      "GET"
    );

    const { data } = category;
    res = data.data;
    var _products = await superPower(
      `${process.env.NEXT_PUBLIC_SERVER_URL}/subcategory/super/${res[0].id}`,
      "GET"
    );
    ...
    __products &&
      __products.forEach((ele) => (products = products.concat(ele.products)));
    mode = "category";
    if (res === null) return redirectPayload;
  } else if (params.length == 2) {
    const subcategory = await superPower(
      `${process.env.NEXT_PUBLIC_SERVER_URL}/subcategory/${params[1]}`,
      "GET"
    );
