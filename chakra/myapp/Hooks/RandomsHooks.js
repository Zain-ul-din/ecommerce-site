...
const LOCAL_STORAGE_NAME = "MXZUCCYFNRMLOEFMDCOOERIUUKEQ5eA7uB7fXZ6aA6qG1hN1yUzxZN5yE3eO5kD2xZO3qN3tF5xZWzdT4bZ3yO7mK5vCART_STORAGE"
...
export function useCart () {
       ...
       switch (action.type) {
        ...
        case 'Init': 
          return action.payload.map ((val) => {
            for (let product of state) if (parseInt(val.id) === parseInt(product.id) && val.countInStock > 0) return {...val , qt : product.qt , totalBill : product.totalBill} 
            return val
          })
        
        case 'Add':
            if (state.filter(val => val.id === action.payload.id).length > 0) return state
        ...
        case 'Update':
            let newState = state.map ((val) => {
                if (val.id === action.payload.id)
                  return {...val , qt : action.payload.qt , totalBill : action.payload.totalBill}
                return val  
            })
    ...
    useEffect (() => {
        ...
        async function fetchProducts () {
             ...
             try { var ids = JSON.parse (window.localStorage.getItem (LOCAL_STORAGE_NAME)) } catch (err) 
             { return saveToLocalStorage ([]) }
             if (!Array.isArray (ids)) return
            
            const res = await httpiepower.get (`${process.env.NEXT_PUBLIC_SERVER_URL}/product/ids` , { params : { ids } })
            dispatch ({type : 'Init' , payload : res.data})
            saveToLocalStorage (res.data)
        }
        
        // listening for onProductChange Event
        socket.on (EVENT_NAME , (message) => { fetchProducts () })
