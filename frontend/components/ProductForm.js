...
function reducer(state, action) {
  switch (action.type) {
    ...
    case "price":
      return {
        ...state,
        data: { ...state.data, price: Math.abs(action.payload) },
        errors: {
          ...state.errors,
          price: {
    ...
    case "countInStock":
      return {
        ...state,
        data: { ...state.data, countInStock: action.payload },
        errors: {
          ...state.errors,
          countInStock: {
            error: action.payload.length > 0 ? undefined : "🎇",
    ...
    case "tags":
      return {
        ...
        errors: { ...state.errors, tags: { error: undefined, message: "" } },
      };
    case "brand":
      return {
        ...state,
        data: { ...state.data, brand: action.payload },
        errors: {
          ...state.errors,
          brand: {
            error: action.payload.length > 1 ? undefined : "🎇",
  ...
  async function handleUpload() {
    ...
    await uploadFile(rstate.data.image, async (imageUrl) => {
      ...
      const res = await axios
        .post(`${process.env.NEXT_PUBLIC_SERVER_URL}/product`, {
  ...
  async function handleUpdate() {
    ...
    if (data.image !== null) {
      ...
      await uploadFile(data.image, async (imageUrl) => {
        ...
        const res = await axios
          .put(`${process.env.NEXT_PUBLIC_SERVER_URL}/product/${data.id}`, {
          ...
          .catch(async (err) => {
            ...
            Toaster(
              toast,
              "data upload fail Internet Connection Error..",
              "error"
            );
          });
      ...
      try {
        const res = await axios.put(
          `${process.env.NEXT_PUBLIC_SERVER_URL}/product/${data.id}`,
          {
            product: data,
          }
        );
          ...
          {props.mode !== "update" && (
            <Menu>
              <MenuButton
                ...
                size={"lg"}
                rightIcon={
                  rstate.data.category_id ? <CheckIcon /> : <ChevronDownIcon />
                }
                ...
                {props.categories &&
                  props.categories.map((category, key) => {
                    return (
                      ...
                      >
                        {category.categories &&
                          category.categories.map((val, idx) => {
                            return (
                              <MenuItem
                                icon={
                                  <CheckIcon
                                    opacity={
                                      rstate.data?.category_id === val.id
                                        ? 1
                                        : 0
                                    }
                                  />
                        ...
                        {category.categories.length === 0 && (
                          <Link href={"/category/upload"}>
                            <MenuItem color={"blue.400"}>Add +</MenuItem>
                          </Link>
                        )}
                        <MenuDivider />
                      </MenuGroup>
