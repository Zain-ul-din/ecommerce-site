...
export default function LoginForm({ mode = "signin" | "signup" }) {
  ...
  async function handleSignIn(email) {
    const res = await httpPower(
      `${process.env.NEXT_PUBLIC_SERVER_URL}/user/${email}`,
      "GET"
    );
    const { data } = res.data;
  ...
  async function HandleSignUp() {
    if (!state.name || state.email === null) {
      ...
      if (state.name && state.name.trim().length < 2)
        Toaster(toast, "user name is too short", "error");
      return;
    }

    // checks user already in record
    const res = await httpPower(
      `${process.env.NEXT_PUBLIC_SERVER_URL}/user/${state.email}`,
      "GET"
    );
    const { data } = res.data;
              ...
              <HStack align={"center"}>
                <Center px={8}>
                  <Box
                    padding={2}
                    role={"button"}
                    onClick={async (e) => SignInWithGoogle()}
                  >
                  ...
                  <Box
                    padding={2}
                    role={"button"}
                    onClick={async (e) => SignInWithFacebook()}
                  >
