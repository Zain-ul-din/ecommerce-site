...
async function getUnique(req, res) {
  ...
  res.send(
    Object.assign(RESPONSE, {
      status: product.length == 0 ? 404 : 200,
      data: product.length == 0 ? null : product,
    })
  );
}
...
async function post(req, res) {
  ...
  const categoryName = await prisma.category
    .findUnique({ where: { id: parseInt(product.category_id) } })
    .catch((e) => e);

  const newProduct = await prisma.product
    .create({
      data: {
        createdAt: isValidDate(new Date(product.createdAt))
          ? product.createdAt
          : new Date().toString(),
        updatedAt: isValidDate(new Date(product.updatedAt))
          ? product.updatedAt
          : new Date().toString(),
        name: product.name,
...
async function updateUnique(req, res) {
  ...
  if (product.image === null) product.image = prev_product.image;
    ...
    // delete prev image
    try {
      fs.unlinkSync(`${process.cwd()}/static/images/${prev_product.image}`);
    } catch (err) {}
  }
        ...
        rating:
          product.rating === undefined
            ? prisma.review.findMany({ where: { id: id } }) === []
              ? parseFloat(
                  (
                    await prisma.review.findMany({ where: { id: id } })
                  ).reduce((prev, acc) => {
                    ratingCount += 1;
