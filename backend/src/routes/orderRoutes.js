    ...
    const orderDetail = await prisma.shippingAddress.create({
      data: {
        ...
        postalCode: !orderDetails.postalCode
          ? 1111
          : parseInt(orderDetails.postalCode),
        addressGoogle: !orderDetails.addressGoogle
          ? "1,1"
          : orderDetails.addressGoogle,
      },
    });
...
async function getSingle(req, res) {
  ...
  res.send(
    Object.assign(RESPONSE, {
      status: order.length == 0 ? 404 : 200,
      data: order.length == 0 ? null : order,
    })
  );
}

orderRouter.get("/:id", getSingle).get("/", getAll).post("/", postOrder);
