...
async function getUnique(req, res) {
  ...
  if (user.length !== 0) {
    ...
    res.setHeader(
      "Set-Cookie",
      cookie.serialize(process.env.COOKIE_NAME, addSalt(token, "http"), {
        httpOnly: false,
        sameSite: "lax",
        maxAge: 24 * 60 * 60,
        path: "/",
  ...
  res.send(
    Object.assign(RESPONSE, {
      status: user.length == 0 ? 404 : 200,
      data: user.length == 0 ? null : user,
    })
  );
}
...
async function post(req, res) {
  ...
  const newUser = await prisma.user
    .create({
      data: {
        ...
        createdAt: isValidDate(new Date(user.createdAt))
          ? user.createdAt
          : new Date().toString(),
        updatedAt: isValidDate(new Date(user.updatedAt))
          ? user.updatedAt
          : new Date().toString(),
  ...
  res.send(
    Object.assign(RESPONSE, { data: newUser, status: HTTP_RESPONSE.created })
  );
}
...
async function updateUnique(req, res) {
  ...
  res.send(
    Object.assign(RESPONSE, { data: updatedUser, status: HTTP_RESPONSE.ok })
  );
}
